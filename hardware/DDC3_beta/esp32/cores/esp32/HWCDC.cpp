...
static void usbEventCallback1(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data){
...
volatile uint16_t user_txBufferSize = 1024;
...
bool HWCDC::connected(){
	return isConnected;
}

void HWCDC::begin(unsigned long baud)
{
	delay(500); // If this delay isn't there, the device will always be seen as connected at startup until reset
    HWCDC::onEvent(usbEventCallback1); // handle CDC status changes
    if(tx_lock == NULL) {
        tx_lock = xSemaphoreCreateMutex();
    }
    setRxBufferSize(user_txBufferSize);//default if not preset
    setTxBufferSize(user_txBufferSize);//default if not preset
    ...
    usb_serial_jtag_ll_txfifo_flush();
	delay(25); // there's a delay before it's actually ready to print. Data will be lost without this
}
