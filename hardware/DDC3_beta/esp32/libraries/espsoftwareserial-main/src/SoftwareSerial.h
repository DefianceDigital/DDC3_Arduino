...
public:
    static constexpr bool isValidPin(int8_t pin) {
        ...
        //return (pin >= 0 && pin <= 1) || (pin >= 3 && pin <= 7) || (pin >= 18 && pin <= 21); // ORIGINAL (Can NOT use I2C as Serial)
		return (pin >= 0 && pin <= 9) || (pin >= 18 && pin <= 21); // MODIFIED for rIOT-LITE (CAN use I2C as Serial)
    #else
        return pin >= 0;
    #endif
    #else
